...
import { auth, db, signInWithEmail, signOutUser } from './firebase';
...
function CompanyToggle({ active, onChange }: { active: CompanyType; onChange: (c: CompanyType) => void }) {
  return (
    <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border-2 border-stone-200 shadow-sm">
      <button
        onClick={() => onChange('transports')}
        className={cn(
...
// ─── Login Screen ──────────────────────────────────────────────────────────────
function LoginScreen({ onLogin, loading, error }: { onLogin: (email: string, password: string) => void; loading: boolean; error: string | null }) {
  ...
  return (
    <div className="min-h-screen bg-emerald-900 flex flex-col items-center justify-center p-6">
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl">
        ...
        <p className="text-stone-400 text-sm mb-8">Sign in to access the transport management system</p>

        {error && (
          <div className="mb-4 p-3 bg-rose-50 text-rose-600 rounded-xl text-sm font-medium">{error}</div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-stone-500 uppercase tracking-wide mb-2">Password</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-900/20"
          >
            {loading ? (
...
export default function App() {
  ...
  const stats = {
    total: consignments.filter(c => c.company === activeCompany).length,
    pending: consignments.filter(c => c.company === activeCompany && c.payment_status === 'Pending').length,
    totalFreight: consignments.filter(c => c.company === activeCompany).reduce((a, c) => a + Number(c.freight_amount || 0), 0),
    totalBalance: consignments.filter(c => c.company === activeCompany).reduce((a, c) => a + Number(c.balance_amount || 0), 0),
  };
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans">
      ...
      <main className="lg:ml-64 min-h-screen pb-28 lg:pb-8">
        {/* Mobile header */}
        <header className="sticky top-0 bg-white/95 backdrop-blur border-b border-stone-100 px-4 py-3 flex flex-col gap-3 lg:hidden z-30">
          ...
          <div className="flex items-center gap-2">
            <CompanyToggle active={activeCompany} onChange={setActiveCompany} />
            <div className="relative flex-1">
              <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-stone-400" />
              <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                className="pl-8 pr-3 py-1.5 border border-stone-200 rounded-lg text-xs w-full focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
            </div>
          </div>
        </header>
        {/* Desktop header */}
